...
# CSV 파일 불러오기
df = pd.read_csv('csv/BigmacPrice.csv')
...
# 제목과 설명 추가
st.title("빅맥가격 추이")
st.write('2000년 ~ 2020년의 백맥가격($) 연평균을 점의 움직임으로 시각화')
...
country_color_map = {country: colors[i % len(colors)] for i, country in enumerate(selected_countries)}
...
# 레이아웃 설정
fig.update_layout(
    title='연도별 국가별 빅맥($) 가격 평균',
    xaxis_title='연도',
    yaxis_title='빅맥($) 가격 평균',
...
# 테이블 표시
st.subheader('연도별 국가들의 빅맥($) 가격 평균 테이블')
