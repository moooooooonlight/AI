...
# CSV 파일 불러오기
df = pd.read_csv('WorldPrice.csv')

# 데이터 프레임을 긴 형식으로 변환
df_long = df.melt(id_vars=['date'], 
                  value_vars=['Korea', 'Australia', 'China', 'India', 'Indonesia', 'Russia', 'South Africa', 'Switzerland', 'United States'],
                  var_name='country', 
...
# 제목과 설명 추가
st.title('세계 물가 지수 시각화')
st.write('여러 국가의 연도별 물가 지수 평균을 점의 움직임으로 시각화')

# 국가 리스트 (기본적으로 모든 국가 선택)
country_list = yearly_avg_df['country'].unique()

# 선택된 국가에 따른 데이터 필터링 (모든 국가가 기본적으로 선택됨)
filtered_df = yearly_avg_df
